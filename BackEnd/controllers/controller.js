...
exports.validateLogin = async (req, res) => {
  try {
    ...
    if (
      user != null &&
      cryptoProvider.comparePassword(req.body.password, user.password)
    ) {
    ...
    res.status(500).json({
      status: "failure",
      message: "exception while validating Login - Bad Request : " + ex,
    });
  }
};
exports.register = async (req, res) => {
  try {
    ...
    res.status(400).json({
      status: "failure",
      message: "exception while insertion - Bad Request : " + ex,
    });
  }
};
exports.getProfile = async (req, res) => {
  try {
    ...
    let user = await userModel
      .findOne({ _id: req.loggedUser._id }, projection)
      .lean();
...
exports.patchUserAddress = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: "success", message: "profile modified successfully" });
  } catch (ex) {
    res.status(500).json({
      status: "failure",
      message: "exception while retrieving the products : " + ex,
...
exports.patchUserImage = async (req, res) => {
  try {
    (await userModel
      .updateOne(
        { _id: req.loggedUser._id },
        { profileImage: req.body.profile.image }
      )
      .exec()) != null;
    res.status(200).json({
      status: "success",
...
exports.deleteUserImage = async (req, res) => {
  try {
    (await userModel
      .updateOne({ _id: req.loggedUser._id }, { profileImage: "" })
      .exec()) != null;
    res.status(200).json({
      status: "success",
...
exports.getProductsById = async (req, res) => {
  try {
    ...
    res.status(500).json({
      status: "failure",
      message: "exception while adding the product : " + ex,
    });
  }
};
...
exports.editProduct = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: "success", message: "product modified successfully" });
  } catch (ex) {
    res.status(500).json({
      status: "failure",
      message: "exception while modifying the product : " + ex,
    });
  }
};
...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    res.status(500).json({
      status: "failure",
      message: "exception while deleting the product : " + ex,
    });
  }
};
...
exports.checkOut = async (req, res) => {
  try {
    ...
    if (req.loggedUser != null) {
      ...
      order.userEmail = req.loggedUser.email;
    } else {
      var hashedPassword = cryptoProvider.encryptPassword(
    ...
    res.status(500).json({
      status: "failure",
      message: "exception while checking out the order : " + ex,
    });
  }
};
exports.getOrders = async (req, res) => {
  try {
    ...
    res.status(500).json({
      status: "failure",
      message: "exception while retrieving the orders : " + ex,
    });
  }
};
...
exports.getOrdersDetails = async (req, res) => {
  try {
    ...
    orderDetails = await orderModel
      .findOne({ user: req.loggedUser._id, _id: req.params.id })
      .lean();
    res.status(200).json({ status: "success", order: orderDetails });
  } catch (ex) {
    res.status(500).json({
      status: "failure",
      message: "exception while retrieving the order details : " + ex,
    });
  }
};
...
exports.editOrder = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: "success", message: "order modified successfully" });
  } catch (ex) {
    res.status(500).json({
      status: "failure",
      message: "exception while modifying the order : " + ex,
    });
  }
};
  ...
  try {
    ...
    if (deletionResponse.deletedCount === 1) {
      ...
      res.status(404).json({
        status: "failure",
        message: "failure",
        message: "No order exists to delete",
      });
    }
  } catch (ex) {
    res.status(500).json({
      status: "failure",
      message: "exception while deleting the order : " + ex,
    });
  }
};
